...
public class CrazyGenerics {
    ...
    static class CollectionUtil {
        ...
        public static boolean hasNewEntities(Collection<? extends BaseEntity> entities) {
            return entities.stream().anyMatch(entity -> null == entity.getUuid());
        }
         ...
         */
        public static <T extends BaseEntity> boolean hasDuplicates(List<T> entities, T entity) {
            return entities.stream().filter(ent -> ent.getUuid().equals(entity.getUuid())).count() > 1;
        }
        ...
        public static <T> Optional<T> findMax(Iterable<T> elements, Comparator<? super T> comparator) {
            ...
            while (iterator.hasNext()) {
                ...
                if (comparator.compare(nextValue, max) > 0) {
                    max = nextValue;
                }
            }
